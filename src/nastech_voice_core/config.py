"""Configuration and constants for Nastech Voice Core TTS package.
...
#
# `revision` pins the HuggingFace Hub commit each Nastech Voice Core release was
...
def get_model_config(model_name: str) -> dict:
    ...
    if model_name not in MODEL_CONFIGS:
        raise ValueError(
            f"Invalid model: '{model_name}'. Available models: {', '.join(AVAILABLE_MODELS)}"
        )
