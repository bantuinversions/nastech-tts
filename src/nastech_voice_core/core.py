...
This module contains the main Nastech Voice Core TTS engine, text processor,
...
class UnicodeProcessor:
    ...
    def _load_indexer(self, unicode_indexer_path: str) -> list:
        try:
            with open(unicode_indexer_path, "r") as f:
                ...
                logger.info(
                    f"Loaded unicode indexer from {unicode_indexer_path} ({len(indexer)} entries)"
                )
    ...
    def _add_language_token(self, text: str, lang: str) -> str:
        ...
        if lang not in AVAILABLE_LANGUAGES:
            raise ValueError(
                f"Invalid language: '{lang}'. Supported languages: {', '.join(AVAILABLE_LANGUAGES)}"
            )
    ...
    def _preprocess_text(self, text: str, lang: Optional[str] = None) -> str:
        ...
        Args:
            text: Raw input text
            lang: Language code for multilingual support. Nastech Voice Core
                supports 31 ISO codes plus the ``"na"`` fallback for
                unknown languages; Nastech Voice Core-2 supports
                ``en``, ``ko``, ``es``, ``pt``, ``fr``.
...
class NastechVoiceCore:
    """Core TTS engine for Nastech Voice Core speech synthesis.
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized Nastech Voice Core engine (sample_rate={self.sample_rate}Hz, "
