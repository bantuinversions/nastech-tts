...
This module handles downloading, loading, and managing Nastech Voice Core TTS models
...
from .core import Style, NastechVoiceCore, UnicodeProcessor
...
def get_cache_dir(model_name: Optional[str] = None) -> Path:
    """Get or create the cache directory for Nastech Voice Core models.
...
def download_model(model_dir: Union[Path, str], model_name: Optional[str] = None) -> None:
    """Download Nastech Voice Core model from HuggingFace Hub.
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to import huggingface_hub. Please install it with: pip install huggingface-hub"
        ) from e
...
def load_configs(model_dir: Union[Path, str]) -> dict:
    ...
    try:
        ...
        raise ValueError(
            f"Model configuration file is malformed at {cfg_path}. Please re-download the model."
        ) from e
...
def load_onnx_modules(
    ...
    if not valid_providers:
        logger.warning(
            f"Requested providers {providers} not available. Falling back to CPUExecutionProvider"
        )
...
def load_model(
    ...
    """Load the complete Nastech Voice Core TTS model.
    ...
    Returns:
        Initialized Nastech Voice Core TTS engine
    """
    ...
    return NastechVoiceCore(cfgs, text_processor, dp_ort, text_enc_ort, vector_est_ort, vocoder_ort)
...
def load_voice_style_from_json_file(voice_style_path: Union[Path, str]) -> Style:
    ...
    try:
        ...
        raise ValueError(
            f"Voice style file is malformed at {voice_style_path}. Please check the file format."
        ) from e
...
def load_voice_style_from_name(model_dir: Union[Path, str], voice_name: str) -> Style:
    ...
    if not voice_style_path.exists():
        ...
        raise FileNotFoundError(
            f"Voice style '{voice_name}' not found. Available voice styles: {', '.join(available)}"
        )
