...
Nastech TTS adds a lazy Bantu model-pack registry. Startup performs zero optional downloads and zero optional model loads; an operator can inspect pack states and explicitly download one requested language through REST, CLI, or agent tools. The `mms-lazy` provider loads one requested local MMS model at a time, evicts the previous model from RAM, and uses the shared CPU/GPU hardware planner.
...
Nastech TTS now reports an automatic hardware plan through the package and REST diagnostics. It detects CPU count, host RAM, CUDA availability, ONNX providers, safe precision, bounded concurrency, and batch recommendations; `NASTECH_DEVICE=gpu` fails closed unless a real CUDA path is available. The compact CPU core remains verified and does not require a GPU.
...
Nastech TTS has been rebuilt as **Nastech TTS**, a real local Nastech Voice Core runtime with an English NastechML agent API. The external Fish S2 provider architecture, GPU requirement, and cloud-provider dependency have been removed.

The compact system performs local CPU synthesis, serves `POST /v1/agent/compile`, `POST /v1/agent/speech`, and an OpenAI-compatible alias, and produces a behavior manifest for every request. It supports documented direct Nastech Voice Core `<laugh>` and `<sigh>` expression tags while honestly marking named emotions and other native-tag requests as release-dependent until acceptance-tested.

The measured complete development deployment subtotal is **647.16 MiB**, under the **1 GiB** budget. This includes 384.83 MiB of actual Nastech Voice Core assets, 260.68 MiB of runtime dependencies, and 1.65 MiB of Nastech release assets.
