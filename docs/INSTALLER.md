...
Nastech Research provides a source-based, one-time bootstrap installer for Linux, macOS, and Windows. When a desktop display and Tk are available, the installer opens a branded animated window with a pulsing Nastech indicator, staged progress, hardware details, dependency status, and clear success/error states. On servers or minimal Python builds without Tk, it automatically falls back to the headless launcher. The installer uses the operating system’s Python launcher, creates an isolated Nastech environment, installs the local package and its declared dependencies, detects the host hardware, persists a runtime profile, and then starts the requested Nastech command.
...
The first non-diagnostic launch creates an isolated environment under the platform’s per-user application-data directory and installs Nastech TTS into it. Later launches reuse that environment. Use `--repair` when dependencies need to be reinstalled or the package must be refreshed. Use `--reset-environment` to recreate the environment from scratch. The Windows wrapper prefers `pyw` so the normal install experience does not expose a console window; Linux and macOS use the animated window when a desktop display is available.

The installer UI hides terminal output from normal users, but the release remains auditable. It does not obfuscate Python source or claim tamper-proof secrecy; logs and source remain available for troubleshooting and review.
