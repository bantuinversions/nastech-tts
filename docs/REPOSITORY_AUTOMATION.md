...
|---|---|---|
| `.github/workflows/ci.yml` | Pushes, pull requests, and manual runs | Runs formatting, static analysis, the 72-test suite on Python 3.10–3.12, OpenAPI drift check, package build, distribution metadata check, and artifact upload |
...
Before relying on hosted CI, run the same core checks locally:
...
The CI workflow also verifies that regenerating `docs/openapi.json` produces no uncommitted schema drift. When an API change is intentional, regenerate the schema locally and commit the updated contract with its tests.
